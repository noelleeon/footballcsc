...
class Entity (object):
    ...
    def _sql_join_all(cls_from, cls_tos):
        ...
        def relation_dists(froms, tos):
            return filter(lambda x: x[2] is not None,
                          ((f, t, dist(f, t)) for f in froms for t in tos))

        def more_general(froms, tos):
            return filter(lambda x: x[2] < 0, relation_dists(froms, tos))
        ...
        while len(tos) > 0:
            ...
            if general:
                f, t, _ = max(general, key=lambda x: x[2])
                joins += add_join(f, t)
            if specific:
                f, t, _ = min(specific, key=lambda x: x[2])
                joins += add_join(f, t)
        return joins
